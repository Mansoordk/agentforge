...
export default function Home() {
  ...
  /*
   * Returns the number of bounties currently visible in the
  ...
  async function getBountyCount() {
    ...
    if (!Number.isFinite(numericCount) || numericCount < 0) {
      throw new Error("Invalid bounty count returned.");
    }
  ...
  async function createBounty() {
    ...
    try {
       ...
       * IMPORTANT:
       *
       * The contract creates bounty IDs using:
       *
       *     bounty_id = len(self.titles)
       *
       * Therefore the bounty count BEFORE creation is
       * exactly the ID that the new bounty will receive.
       *
       * Example:
       * count = 3
       * new bounty ID = 3
       */
      const previousCount = await getBountyCount();
      const newBountyId = previousCount;

      setStatus(
        `Creating bounty #${newBountyId}...`
      );
      ...
      /*
       * Use the ID calculated BEFORE the transaction.
       * Do NOT call getBountyCount() again here because
       * LATEST_FINAL may still show the previous state.
       */
      ...
      setStatus(
        `Bounty #${newBountyId} created. Loading bounty...`
      );

      /*
       * Load the exact bounty we just created.
       */
  ...
  async function settlePartial() {
    ...
    if (bounty.status !== "PARTIAL") {
      setStatus(
        "This bounty does not have a PARTIAL verdict."
      );
      return;
    }
        ...
        <section className="grid">
          ...
          <div className="card" id="create">
            ...
            <input
              ...
              onChange={(e) =>
                setEligibleWorker(e.target.value)
              }
              placeholder="0x1234..."
            />
